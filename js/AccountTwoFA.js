...
function EmailValidate() {  
    if (ValidateInput()) {
        ...
        $.post("/account/verifytwofa", {"email":$("#email").val()}, 
            function (data, textStatus, jqXHR) {
                if(data=="Error-Saving"){
                    ShowError("Err 013: We have problems with saving secrity code. Please contact the site adminstrator");
                }else if(data=="Error-Emailing"){
                    ShowError("Err 014: We have problems with sending email. Please contact the site adminstrator");
        ...
        ShowError("Err 012: Please enter a valid email.");
    }
}
...
function SubmitCodeEmail(){
    ...
    $.post("/account/twofaset", {"code":code,"set":"email","email":email}, 
            function (data, textStatus, jqXHR) {
                if(data=="Error-Matching"){
                    ShowError("Err 015: You have entered code wrong.");
                }else if(data=="Error-Saving"){
                    ShowError("Err 016: We have problems with saving your Two-Factor Authentication. Please contact the site adminstrator");
                }else if(data=="Error-Editing-Account"){
                    ShowError("Err 017: Had problem with changing your information. Please contact the administrator.");
                }else if(data=="Success"){
